    ...
    return events_in.map((availability) => {
        ...
        if (availability !== undefined) {
            if (availability.bookedByConfirmed) {
                status = "booked";
                color = "#FF0000";
                if (moment(availability.startDate)>moment()){
            ...
            if (private_in) {
                ...
                if (status === "booked" || status === "booked active" || status === "reserved") {
...
this.calendarClickOptions = function (calEvent, pageSession) {
    if (calEvent.private) {
        if (calEvent.status === "booked" || calEvent.status === "reserved" || calEvent.status === "booked active") {
