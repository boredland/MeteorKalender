...
then
  ...
  RESULT_MESSAGE="Sucessfully deployed <"$DESTINATION_URL"|#"$TRAVIS_BUILD_NUMBER"> for <https://github.com/boredland/MeteorKalender/commit/"${echo $TRAVIS_COMMIT | cut -c -7}"|"$TRAVIS_COMMIT">."
else
